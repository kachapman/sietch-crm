...
- **Mail sidebar folders are now scoped to the active tab.** `renderMailFolderList()` passes the active account ID to `GET /api/v2/mail/folders?account_id=...`. The server returns only folders belonging to the selected account (or all CRM mail accounts when `account_id=crm`). Previously, the sidebar showed the same union of all folders for every tab.
