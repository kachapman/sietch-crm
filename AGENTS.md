...
**Last session summary (for next resume):** 2026-07-26 session 25: (1) Fixed `/project/{id}` URL rewrite — changed from silent internal rewrite to 302 redirect so browser URL gets `?id=` in query string. (2) Fixed stage showing `[object Object]` — API returns `{title, id}` object, now extracts `.title`. (3) Added @-mention autocomplete to project.html note editor — fetches portal users, dropdown on `@`, keyboard nav (Enter/Tab/Escape), colored mention chips, extracts user IDs on submit and passes `notifyUserList`. Fixed `sel` scope bug in mousedown handler. (4) Added user fields to project page — fetches defs from `/api/v2/custom-fields` + values from `/api/v2/projects/{id}/custom-fields`, cross-references for proper labels. (5) Fixed user field save from deal-edit and preview modals — `PUT /api/v2/projects/{id}` now handles `customFieldList` (was silently ignored). Fixed FK constraint violation: `contactId:0`/`responsibleUserId:0` converted to NULL. Fixed `customFieldTypeCode` to also check `def.type` (server returns `{type:3}` not `{fieldType:3}`), fixing checkbox display. Fixed checkbox uncheck not persisting — unchecked now sends `"false"` instead of skipping. Files: `public/project.html`, `public/app.js`, `server.py`. Commits `417f18f`, `59e7b74`, `e2e033e`, `385dd68`, `26cc475`, `b76d8df`.
