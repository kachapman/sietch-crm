...
**Last session summary (for next resume):** 2026-07-31 session 44 (cont.): Fixed OAuth send/IMAP failure + CRM Mail tab isolation. (1) Root cause of `530 5.7.57 Client not authenticated` + `AUTHENTICATE failed`: `MicrosoftProvider._SCOPE` (oauth_providers.py:81) mixed Graph `User.Read` with Exchange scopes, so the v2.0 endpoint minted a **Graph-audience** access token (`aud=00000003-0000-0000-c000-000000000000`, confirmed by decoding stored token) which Exchange rejects for IMAP/SMTP. Dropped `User.Read` → scope is now `offline_access openid email https://outlook.office.com/IMAP.AccessAsUser.All https://outlook.office.com/SMTP.Send`, so the token audience is the Exchange resource. Also hardened `exchange_code()` email extraction (prefer `email` claim, strip `live.com#` prefix from `preferred_username`). (2) CRM Mail account no longer renders as its own tab: `_handle_mail_accounts()` now returns `is_crm_mail`, and `renderMailTabs()` skips `is_crm_mail === true` accounts. (3) `account_id=crm` sentinel now filters to `is_crm_mail = TRUE` accounts in messages/threads/drafts/unread-count (was "all"); frontend sends `account_id=crm` from the CRM Mail tab + queries its unread badge. (4) **SECOND root cause found after re-consent:** account `vanguardadjusting@outlook.com` is a **personal live.com/MSA invited as a B2B guest** (#EXT# UPN) into tenant 7312e555, so even a correctly-scoped token issued by the host tenant (iss `sts.windows.net/7312e555/`) is rejected by Exchange Online for IMAP (AUTHENTICATE failed) and SMTP (535 5.7.3). Fixed by switching `MicrosoftProvider._tenant()` to **`common`** so the MSA authenticates through the consumers STS (iss `sts.windows.net/9188040d-...`). REQUIRES user Azure-portal change: app registration "Sietch CRM" → Supported account types → "Accounts in any organizational directory and personal Microsoft accounts", then re-run Email → Admin → OAuth Connect → Accept. User confirmed IMAP/POP/SMTP settings are already ON on the account. (5) Added `SCANNER_SYNC_DAYS` (default 90, config.example.env + `_poll_mailboxes` in mail_scanner.py) — scanner was fetching the ENTIRE inbox every 5-min poll (no `since`); now bounded via IMAP SINCE search (0/negative = full history). Server restarted (PID 2419330), health 200, authorize_url now `login.microsoftonline.com/common/...`. Prior session (43) fixed the import-order bug + email list double-height rows via container query.
