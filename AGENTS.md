...
**Last session summary (for next resume):** 2026-07-31 session 44: Fixed OAuth send/IMAP failure + CRM Mail tab isolation. (1) Root cause of `530 5.7.57 Client not authenticated` + `AUTHENTICATE failed`: `MicrosoftProvider._SCOPE` (oauth_providers.py:81) mixed Graph `User.Read` with Exchange scopes, so the v2.0 endpoint minted a **Graph-audience** access token (`aud=00000003-0000-0000-c000-000000000000`, confirmed by decoding stored token) which Exchange rejects for IMAP/SMTP. Dropped `User.Read` → scope is now `offline_access openid email https://outlook.office.com/IMAP.AccessAsUser.All https://outlook.office.com/SMTP.Send`, so the token audience is the Exchange resource. Also hardened `exchange_code()` email extraction (prefer `email` claim, strip `live.com#` prefix from `preferred_username`). (2) CRM Mail account no longer renders as its own tab: `_handle_mail_accounts()` now returns `is_crm_mail`, and `renderMailTabs()` skips `is_crm_mail === true` accounts. (3) `account_id=crm` sentinel now filters to `is_crm_mail = TRUE` accounts in messages/threads/drafts/unread-count (was "all"); frontend sends `account_id=crm` from the CRM Mail tab + queries its unread badge. IMPORTANT: re-consent is REQUIRED — old consent returns `AADSTS65001 consent_required` on refresh (verified; nothing written to DB). User must re-run Email → Admin → OAuth Connect and ACCEPT to mint a valid Exchange-audience token before IMAP/SMTP works. Server restarted (PID 2336781), health 200, scope verified in authorize_url. Prior session (43) fixed the import-order bug + email list double-height rows via container query.
