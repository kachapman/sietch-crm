...
**Last session summary (for next resume):** 2026-07-28 session 30: Final Phase 3 polish. (1) Duplicate folders root-cause fixed — DB had 12 duplicate rows, cleaned to 6; added missing columns to `mail_folders` table. (2) Scanner log terminal UI — monospace, dark background, color-coded status entries. (3) Template dropdown fix — click-outside closes, re-click removes previous picker. (4) Dark mode contrast — removed `color: #222` override, defined `--bg-elevated`. (5) [#PROJECTID] auto-link wired — `DEAL_LINK_RE` now used, toggle added (default ON). (6) CRM Mail Accounts — renamed section, added SMTP fields to quick-add form. (7) Helper tooltips on all form fields. Phase 3 is now functionally complete. OAuth2 deferred. Files changed: `public/app.js`, `public/index.html`, `public/styles.css`, `scanner/mail_scanner.py`.
