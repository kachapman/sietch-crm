...
**Last session summary (for next resume):** 2026-07-24 session 19: Fixed notification trigger timing race and reorganized group tile toolbar. (1) Replaced PostgreSQL `AFTER INSERT` trigger on `history_events` with explicit Python notification creation in `_handle_project_history_create()` — the trigger fired during `db.insert_returning()` before `history_notify_users` was populated, so zero notifications were ever created. Removed `create_tag_notifications` function and trigger from `init.sql`. (2) Moved filter toggle and template buttons from `.tile-toolbar-tools` wrapper to direct toolbar children, positioned right of the title text. On narrow tiles (<550px), deal count and filter summary wrap to a second row while buttons stay with the title. Close/minimize always pinned far right. Files: `server.py`, `init.sql`, `public/app.js`, `public/styles.css`. Cache-bust bumped to v2.1.0. NOT YET PUSHED.
