...
**Last session summary (for next resume):** 2026-07-28 session 36: Email bug fixes + full preview modal. (1) Email link error handling — server validates opportunity exists, returns specific errors; frontend surfaces server error messages. (2) Hover popover disabled on mobile (≤600px + pointer:coarse). (3) Mobile action buttons enlarged (36px min-touch, 1.1rem icons). (4) Note button fallback — opens compose modal prefilled when outside opp preview. (5) Full email preview popup modal (`#email-preview-modal`) — replaces mobile overlay + desktop inline expansion; shows full email, attachments with download links, action toolbar (Reply/Reply All/Forward/Note/Archive/Print/Headers). (6) Attachment download endpoint — lazy-fetches from IMAP via `imap_tools`, streams binary response. (7) Image paste/drag-drop in compose body — detects clipboard images and dropped files, adds to attachments. Files changed: `public/app.js`, `public/index.html`, `public/styles.css`, `server.py`, `CHANGELOG.md`, `AGENTS.md`.
