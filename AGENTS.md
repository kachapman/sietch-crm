...
**Last session summary (for next resume):** 2026-07-24 session 23: (1) Unified note editor formatting toolbar across preview modal, deal-edit modal, and quick-note modal: all now have B I U S | • 1. | 🖍 😀 🔗 | ⌫ | [calendar]. (2) Moved backdate calendar button into formatting toolbar as last button (removed separate `.note-backdate-wrap`). (3) Preview modal section reordered: History & notes title → Quick Context button → Add Note editor → History list. (4) Fixed "strange semi-colon" text node by switching template literal to string concat (no leading newline). (5) Added global delegated handlers for `.note-format-hilite-btn`, `.note-format-emoji-btn`, `.note-format-link-btn` so they work in all editors. (6) Fixed group tile count + filter summary wrapping: wrapped in `.group-tile-count-summary` flex container so they stay on one line. (7) Changed Add Tile header button icon to `layout-grid-add` (3 squares + plus). (8) Removed stale `.note-backdate-wrap`/`.note-backdate-label` CSS. Files: `public/app.js`, `public/styles.css`, `public/index.html`, `AGENTS.md`, `CHANGELOG.md`. Cache-bust bumped to v2.2.0.
