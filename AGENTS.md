...
**Last session summary (for next resume):** 2026-07-24 session 18: Fixed two critical regressions and expanded tile resize capabilities. (1) Fixed @-mention notifications never created: `isGuid()` filter on `notifyUserList` silently rejected integer user IDs — now accepts both GUIDs and numeric IDs. (2) Fixed dashboard tile grid snapping: panel tiles (feed/tasks/presence) had `min-height: 0` → restored to `var(--tile-row-height)` (400px); group tiles capped at `max-height: var(--tile-row-height)` with scrollable filter header and board. (3) Multi-row tile resize: tiles can now be 3-8 rows tall (was binary normal/double), using continuous row count from CSS variable. (4) Three-column width enabled: removed span-3 ban, `tile-three` CSS class now functional. (5) Cleaned up @-mention dead code: removed `_mentionSyncHidden()`, `.mention-chips` containers, hidden inputs. (6) Fixed bot user display names. Files: `public/app.js`, `public/styles.css`, `public/index.html`. Cache-bust bumped to v2.1.0. NOT YET PUSHED.
