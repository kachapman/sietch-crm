...
**Last session summary (for next resume):** 2026-07-31 session 43: Fixed the persistent "No OAuth providers configured" error. Root cause: `server.py` imported `mail_scanner` BEFORE `_load_env_file()`, so `mail_scanner`'s `import oauth_providers` baked in empty env-derived constants. Moved the `import mail_scanner` block in `server.py` to after `_load_env_file()`/`import oauth_providers` (with a comment explaining the ordering constraint). Verified end-to-end: `GET /api/v2/mail/oauth/authorize?provider=microsoft` now returns a real `authUrl` (tenant `7312e555-...`, client `e285c2a5-...`, redirect to `https://g2vpdgb498.localto.net/api/v2/mail/oauth/callback`). Also fixed email list double-height rows: `.mail-list-container` now `container-type: inline-size` and an appended `@container (max-width: 720px)` block in styles.css forces the body snippet onto its own second line when the list's actual rendered width is too narrow (from+subject stay on line 1). Root cause of the earlier CSS failure: the base `.mail-snippet { flex-basis:0; order:5 }` rule at styles.css ~11440 was overriding the mobile media-query rule because it comes later in the file (same specificity); the new container-query block is at EOF so it wins. User still needs to re-run the Microsoft 365 Connect flow and ACCEPT the full consent screen (old consent was missing new scopes → AADSTS65001) before IMAP/SMTP will work.
