...
**Last session summary (for next resume):** 2026-07-28 session 38: Compose tab feature. Added "Move to tab" button to compose modal header (tab icon SVG). When pressed, closes compose modal and creates a new compose tab in the mail inbox modal tab bar. Each tab shows full compose UI (From+Link side-by-side, To/Cc/Bcc, Subject, formatting toolbar, contenteditable body with image paste/drop, attachments, Send/Cancel). Multiple compose tabs supported. Cancel/× prompt "Discard this draft?". Tab styled with accent background to stand out. Updated `switchMailTab()` to handle compose tab IDs. Files changed: `public/app.js`, `public/index.html`, `public/styles.css`, `CHANGELOG.md`, `AGENTS.md`.
