...
**Last session summary (for next resume):** 2026-07-26 session 26: (1) Fixed checklist checkbox display in preview modal — root cause was `field_type='text'` in DB for Measurement Report, Insurance Documents, Inspection Photos (should be 'checkbox'). Updated DB via SQL. (2) Increased CSS specificity for `.opp-preview-field-chk` rules (0,2,0 beats 0,1,0 from `.opp-preview-field`) so compact styling takes effect. (3) Added safety net in checklist renderer for raw `"true"` values. Commits `213298a`, `302e610`.
