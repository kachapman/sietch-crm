...
**Last session summary (for next resume):** 2026-08-01 session 45: OAuth email now FULLY WORKS for `vanguardadjusting@outlook.com` (IMAP sync + SMTP send) after the `common`-tenant + Exchange-scope fixes from prior sessions. Three remaining bugs fixed and verified E2E: (1) **IMAP fetch returned nothing** — `_fetch_messages()` passed a plain dict `{"date": since}` to imap_tools `mailbox.fetch()`, which stringified to a Python repr and produced `UID SEARCH ... BAD The specified message set is invalid` every poll. Now uses `A()` / `A(date_gte=since.date())` (imap_tools 1.14 requires `datetime.date` not `datetime`; `date_gte` = SINCE, `date` = ON). (2) **`MailMessage` has no `.message_id`** in imap_tools 1.14 — now pulled from `msg.headers['message-id']` (tuple-joined, `<>` stripped; header values in raw_headers also joined). (3) **SMTP send 530 5.7.57** — after `starttls()`, Exchange requires a fresh `EHLO` before `AUTH XOAUTH2`, else it replies `503 5.5.2 Send hello first` and auth silently fails. Added `server.ehlo()` after `server.starttls()` in `smtp_client.py`. (4) **`notification_dispatcher.py` crashed every 5s** (`TypeError: tuple indices ... not str`) — `db.query()` returns tuples but code indexed rows by name; switched to `db.query_dicts()` and `row["enabled"]` in `_user_telegram_enabled()`. VERIFIED: server PID 1139142 (later restarts), health 200; scanner poll syncs INBOX (account id=6) into `mail_messages`; `POST /api/v2/mail/send` returned 200 and the sent message round-tripped back through IMAP. NOTE: the old token is opaque (`EwAo...` JWE) so claims can't be decoded from the DB — no longer needed since both IMAP+SMTP now work. UNCOMMITTED at handoff: init.sql + migrations/001 + new migrations/003_mail_outgoing_cascade.sql (FK CASCADE), scanner/smtp/dispatcher fixes, watchdog files, AGENTS/CHANGELOG. **Previous session (44):** OAuth scope fix (dropped Graph `User.Read`), `_tenant()` → `common` for MSA/B2B guest, `SCANNER_SYNC_DAYS=90`, CRM Mail tab isolation, localtonet tunnel restored (IAM service only), watchdog installed.
