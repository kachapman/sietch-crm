...
**Last session summary (for next resume):** 2026-07-28 session 29: Bug fixes & feature wiring. (1) New Tag button fixed — `renderMailFolders()` now called on modal open. (2) Duplicate folders fixed — server uses `DISTINCT ON (name)` to deduplicate across IMAP accounts. (3) Email body iframe height fixed — removed `min-height: 240px`, lowered JS floor to 40px, cap to 900px. (4) Behavior toggles wired — scanner reads `post_notes` toggle before posting notes. (5) Classification rules wired to scanner — new `_evaluate_classification_rules()` evaluates user-defined rules (subject_regex, sender_domain, body_regex) before hardcoded patterns. (6) Template management for users — "Save as Template" button in compose, templates section in email sidebar. (7) Contractor form UX — account/user dropdowns instead of raw ID numbers, renamed to "New Routing Rule". (8) Admin panel two-column layout — CSS Grid, collapses below 900px. Files changed: `public/app.js`, `public/index.html`, `public/styles.css`, `scanner/mail_scanner.py`, `server.py` (332 insertions, 140 deletions).
