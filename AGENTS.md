...
**Last session summary (for next resume):** 2026-07-26 session 24: (1) Wired "Open in CRM" buttons to open project detail page: `crmOpportunityUrl(id)` now returns `/project/{id}` (was `#`), `crmTaskUrl(task)` extracts oppId and returns `/project/{oppId}` (was `#`). (2) Renamed all "Open in CRM" text to "Open Project" — `createCrmOpenLink()` default title, plus 3 HTML buttons (task preview, deal-edit, quick-note modals). (3) Rewrote `project.html` (390 lines): dark theme via CSS variables from `styles.css`, full project fields (stage, value, contact, due date, responsible, created, description, tags with High Priority highlight, custom fields in 2-column grid), Add Note editor in "History & notes" section header (matching preview modal location), contenteditable with format toolbar (B/I/U/S/lists/clear), submit to `POST /api/v2/projects/{id}/history` with `categoryId: 10`, re-fetches events after submit, mobile responsive (720px max-width). Files: `public/app.js`, `public/index.html`, `public/project.html`, `AGENTS.md`.
